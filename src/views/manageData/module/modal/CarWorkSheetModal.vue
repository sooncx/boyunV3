    ...
    <a-form ref="ruleForm" :model=form :rules="rules" class="carWorkSheetModal" >
      <div class="row">
        <a-form-item ref="name" label="车主姓名" name="carOwner">
          <a-input :disabled="formDisabled" v-model:value.trim="form.carOwner" />
        </a-form-item>
        <a-form-item ref="name" label="联系电话" name="phone">
          <a-input :disabled="formDisabled" v-model:value.trim="form.phone"  />
        </a-form-item>
      </div>
      <div class="row">
...
import { carWorkSheetModalTypeEnum } from '@/enums/manageDataEnum';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    watch(() => props.visible,(value)=>{
      if(!value) return;
      data.title = props.type === carWorkSheetModalTypeEnum.ADD ? '新建工单' : props.type===carWorkSheetModalTypeEnum.EDIT? '修改工单' : '查看工单';
      data.deviceList = [];
      data.formDisabled = false;
      // 初始化表单数据
      initForm();
      if(props.type !== carWorkSheetModalTypeEnum.ADD){
      ...
      // 判断查看类型  不能编辑
      if(props.type === carWorkSheetModalTypeEnum.SHOW) data.formDisabled = true;
      // 获取安装人员列表
      getInstallWorker();
    });
    ...
    const handleOk =  () =>{
      ...
      // 判断是查看 则关闭窗口
      if(props.type === carWorkSheetModalTypeEnum.SHOW){
    ...
    const closeDeviceList = (index:number) => {
      // 查看功能 无法删除
      if(props.type === carWorkSheetModalTypeEnum.SHOW) return;
