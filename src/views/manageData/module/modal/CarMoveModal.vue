    ...
    <a-form :model=form class="modelsff" >
      ...
      <div class="row" v-if="modalType=='table'">
        <a-form-item label="设备号" >
          <a-input v-model:value.trim="form.terminalNo" disabled />
        </a-form-item>
      </div>
      <div class="row" v-if="modalType=='all'">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const handleOk = async () => {
      // 表格里面的转移
      if(props.modalType === carMoveModalTypeEnum.TABLE){
        const info = {
      ...
      // 车辆转移
      if(props.modalType === carMoveModalTypeEnum.ALL){
        const info = {
      ...
      // 转移选中项
      if(props.modalType === carMoveModalTypeEnum.SELECT){
        const info = {
