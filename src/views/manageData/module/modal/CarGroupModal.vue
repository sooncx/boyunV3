    ...
    <a-form :model=form ref="ruleForm" class="modelsff" >
      ...
      <div class="row">
        <a-form-item  label="车组名" >
          <a-input  v-model:value.trim="form.groupName" />
        </a-form-item>
      </div>
      <div class="row">
...
<script lang="ts">
import { defineComponent,inject,reactive,toRefs,PropType,watch,nextTick, computed } from "vue";
  ...
  setup(props, {emit}) {
    ...
    const getData = async () => {
      try {
        ...
        if(flag !== 1) throw msg;
        // 判断修改
        if(props.carGroupModelType === modalTypeEnum.EDIT){
    ...
    watch(()=>props.visible,(value:any)=>{
      ...
      data.title = props.carGroupModelType === modalTypeEnum.EDIT ? '编辑车组数据' : '新增车组'; 
      nextTick(()=>{
        getData();
      });
    ...
    const handleOk = async () => {
      ...
      try {
        ...
        // 判断当前是新增/修改
        if(props.carGroupModelType === modalTypeEnum.ADD){
