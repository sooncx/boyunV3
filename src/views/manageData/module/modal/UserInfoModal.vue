          ...
          <div class="row">
            ...
            <div class="item">
              <a-form-item label="登录密码" name="password">
                <a-input v-if="type !== userInfoModalTypeEnum.EDIT" v-model:value.trim="form.password" />
          ...
          <div class="row">
            <div class="item">
              <a-form-item label="客户名称" name="corpName">
                <a-input v-model:value.trim="form.corpName" />
              </a-form-item>
            </div>
            <div class="item">
          ...
          <div class="row">
            <div class="item">
              <a-form-item label="上级账户" name="parentName">
                <a-input :disabled="true" v-model:value.trim="parentInfo" />
              </a-form-item>
            </div>
            <div class="item">
              <a-form-item label="联系电话" name="phone">
                <a-input v-model:value.trim="form.phone" />
              </a-form-item>
            </div>
          </div>
          <div class="row">
            <a-form-item :style="{width: '100%'}" label="备注信息" name="remark">
              <a-input v-model:value.trim="form.remark" />
            </a-form-item>
          </div>
          <!-- 车组绑定 -->
          <div class="row rowRemark" v-if="type !== userInfoModalTypeEnum.EDIT">
...
import { userInfoModalTypeEnum } from '@/enums/manageDataEnum';
export default defineComponent({
  ...
  setup(props, {emit}) {
    ...
    const submitCheck = () =>{
      ...
      // 0是  1否
      return {
        checkOffOil: Number(!data.form.checkOffOil),
        checkUser: Number(!data.form.checkUser),
        checkGroup: Number(!data.form.checkGroup),
        checkVehicle: Number(!data.form.checkVehicle),
      }
    }
    ...
    async function handleOk(){
      data.ruleForm.validate().then(async () => {
        ...
        try {
          // 判断 是否修改用户信息
          if(props.type === userInfoModalTypeEnum.EDIT){
    ...
    watch(()=>props.visible,(value)=>{
      nextTick(()=>{
        ...
        data.title = props.type === userInfoModalTypeEnum.ADD ? '新增用户' : 
                     props.type === userInfoModalTypeEnum.ADDPARENT ? '新增下级' : '修改用户';
        data.bindGroupNum = 0;
        data.groupIds = '';
        data.parentInfo = '';
        data.groupIds = '';
        ...
        initForm();
        data.rules.password = [];
        if(props.type === userInfoModalTypeEnum.ADD){
