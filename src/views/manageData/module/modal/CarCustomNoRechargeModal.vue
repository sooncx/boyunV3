...
import { customRechargeTypeEnum } from '@/enums/manageDataEnum';
export default defineComponent({
  ...
  props: {
    ...
    customNoRechargeType: {
      type: Number,
      default: customRechargeTypeEnum.CUSTOM_NO_RECHARGE_OUT
    },
    // 显示状态
    visible: {
      type: Boolean,
  ...
  setup(props, { emit }) {
    ...
    async function customNoRechargeHandleOk(){
      // 判断是否自定义续费
      if(props.customNoRechargeType === customRechargeTypeEnum.CUSTOM_RECHARGE){
        customBatchRecharge();
        return;
      }
      ...
      const info = {
        ...
        isState: props.customNoRechargeType
      }
      try{
    ...
    watch(()=>props.visible,()=>{
      switch(props.customNoRechargeType){
        case customRechargeTypeEnum.CUSTOM_NO_RECHARGE:
          data.title = '自定义不续费';
          break;
        case customRechargeTypeEnum.CUSTOM_NO_RECHARGE_OUT:
          data.title = '自定义不续费转出';
          break;
        case customRechargeTypeEnum.CUSTOM_RECHARGE:
          data.title = '自定义续费';
      }
      data.devNo = '';
    });
