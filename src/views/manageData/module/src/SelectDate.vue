<template>
  <div class="selectDay">
    <template v-if="dateType === selectDateTypeEnum.DATE || dateType === selectDateTypeEnum.ALL">
      <a-date-picker
    ...
    <ul class="selectDay__opera" v-if="dateType === selectDateTypeEnum.DAY || dateType === selectDateTypeEnum.ALL || dateType=== selectDateTypeEnum.DAYCUSTOM">
      <li
    ...
    <a-modal
      ...
      @ok="handleOk"
    >
      <a-input v-model:value.trim="customDay" placeholder="请输入天数"></a-input>
    </a-modal>
  </div>
</template>
<script lang="ts">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    if(props.dateType === selectDateTypeEnum.DAYCUSTOM){
      datelist.value.push({
        name: '自定义',
        interval: 0
      });
