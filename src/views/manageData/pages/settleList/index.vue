<template>
  <div class="content">
    ...
    <div class="content__body">
      <div class="content__body__search">
        ...
        <div class="content__body__search__right">
          <a-button v-permission="160501" type="primary" @click="onSearch">
            <template #icon><i class="iconfont icon icon-sousuo"></i></template>搜索
          </a-button>
          <a-button v-permission="160502"  @click="onExport">
          ...
          >
            ...
            <el-table-column prop="button" align="center" label="操作">
              <template #default="scope">
                <div class="setting">
                  <span v-permission="160503" @click="back(scope.row)">结清撤回</span>
                </div>
              </template>
            </el-table-column>
          </el-table>
    ...
    <!-- 结清撤回弹窗 -->
    <CarSettleListModal :vehicleId="vehicleId" @settlementConfirm="settlementConfirm" v-model:visible="carSettleListModalVisible"></CarSettleListModal>
  </div>
</template>
<script lang="ts">
...
export default defineComponent({
  ...
  setup() {
    ...
    const getTotal = async () => {
      try {
        const { obj,flag,msg } = await API.getSettleTotal();
        if(flag !== 1) throw msg;
        data.headerAppend.value = obj.total;
      } catch (error) {
