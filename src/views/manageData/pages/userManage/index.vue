...
import { treeSettingTypeEnum,userInfoModalTypeEnum } from '@/enums/manageDataEnum';
export default defineComponent({
  ...
  setup() {
    ...
    const settingFun = (type:string,item:any) => {
      // 绑定车组
      if(type === treeSettingTypeEnum.EDITINFO) bindCar(item);
      // 添加
      if(type === treeSettingTypeEnum.ADD){
        if(item === null){
          addUser(userInfoModalTypeEnum.ADD);return;
        }
        addUser(userInfoModalTypeEnum.ADDPARENT,item);
      }
      // 修改
      if(type === treeSettingTypeEnum.EDIT) addUser(userInfoModalTypeEnum.EDIT,item);
      // 删除
      if(type === treeSettingTypeEnum.DELETE) deleteUser(item);
    }
