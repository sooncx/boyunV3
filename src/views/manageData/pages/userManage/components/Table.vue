        ...
        <el-table-column
          ...
          <template #default="scope">
            <div class="setting" v-if="scope.row.userId !== stateUserId">
              <span v-permission="160102" @click="model(treeSettingTypeEnum.ADD,scope.row)">新增下级</span>
              <span v-permission="160101" @click="model(treeSettingTypeEnum.EDIT,scope.row)">修改</span>
              <span v-permission="160105" @click="model(treeSettingTypeEnum.EDITINFO,scope.row)">绑车</span>
              <span v-permission="160106" @click="resPassword(scope.row.userId)">重置密码</span>
              <span v-if="checkUserDelete()" v-permission="160104" @click="model(treeSettingTypeEnum.DELETE,scope.row)">删除</span>
            </div>
            <span v-else>
              -
            </span>
...
import { treeSettingTypeEnum } from '@/enums/manageDataEnum';
