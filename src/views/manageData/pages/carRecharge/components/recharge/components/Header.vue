  ...
  <div class="header">
    <div class="header__top">
      <div class="header__top__left">
        ...
        <div class="item" v-if="renew === renewEnum.YES">
          <label>状态</label>
          <a-select
        ...
        <div class="item date" v-if="renew === renewEnum.YES">
          <label>即将到期</label>
          <SelectDate @selectDay="onExpireTime" :dateType="selectDateTypeEnum.DAYCUSTOM"></SelectDate>
          <span @click="changeHighSearch"><MinusOutlined v-if="highSearch"/><PlusOutlined v-else/>高级筛选</span>
        </div>
      </div>
      <div class="header__top__right">
        <a-button v-permission="160401" type="primary" @click="onSearch">
    ...
    <div class="header__bottom" ref="headerBottom" v-show="highSearch">
      <div class="item">
        <label>安装时间</label>
        <div class="group">
          <SelectDate v-model:value="installDate" :dateType="selectDateTypeEnum.DATE"></SelectDate>
        </div>
      </div>
      <div class="item">
        <label>激活时间</label>
        <div class="group">
          <SelectDate v-model:value="activationTime" :dateType="selectDateTypeEnum.DATE"></SelectDate>
        </div>
      </div>
      <div class="item">
        <label>离线时间范围（天）</label>
        <div class="group dateInput">
          <a-input v-model:value.trim="search.offlineTimeBegin" />
          至
          <a-input v-model:value.trim="search.offlineTimeEnd"/>
        </div>
      </div>
      <div class="item">
        <label>续费年限（年）</label>
...
<script lang="ts">
import { defineComponent, inject, Ref, toRefs,reactive,nextTick  } from "vue";
...
import { renewEnum,selectDateTypeEnum } from '@/enums/manageDataEnum';
export default defineComponent({
  ...
  setup(props,{emit}){
    const data = reactive({
      renewOptions: [                             // 续费选择值
        { value: renewEnum.YES,label: '续费' },
        { value: renewEnum.NO,label: '不续费' }
      ],
    ...
    const onExport = async () => {
      const { flag, msg }  = await API.exportVehicleExpire({...data.search});
      if (flag === 1) {
