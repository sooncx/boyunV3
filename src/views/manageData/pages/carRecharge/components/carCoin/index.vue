      ...
      <div class="header__top">
        <div class="header__top__left">
          ...
          <div class="item" v-show="type === ''">
            <label>划拨币数</label>
            <div class="group">
              <a-input v-model:value.trim="userRechargeForm.chargeNum"></a-input>
            </div>
          </div>
          <div class="item button" v-show="type === ''">
            <a-button v-permission="160402" type="primary" @click="userRecharge">划拨</a-button>
          </div>
          <div class="item" v-show="type === 'log'">
            <label>选择日期</label>
            <SelectDate v-model:value="rangeDate" @selectDay="onExpireTime" :dateType="selectDateTypeEnum.ALL" />
          </div>
        </div>
        <div class="header__top__right" v-show="type === 'log'">
          <div class="item button">
            <a-button v-permission="160403" type="primary" @click="onSearch">
              <template #icon><i class="iconfont icon icon-sousuo"></i></template>
              搜索</a-button>
          </div>
          <div class="item button">
            <a-button @click="onExpload" :loading="buttonLoading">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // 导出功能
    const onExpload = async () => {
      data.buttonLoading = true;
      await API.userRechargeLogExport(data.search);
