        ...
        <el-table-column label="操作" fixed="right" width="120" header-align="center">
          <template #default="scope">
            <div class="setting">
              ...
              <span v-permission="160604" v-if="scope.row.status === 3" @click="mode(scope.row,'complete')" >完成</span>
              <span v-permission="160603" v-if="scope.row.status < 3" @click="mode(scope.row,'edit')">修改</span>
              <span v-permission="160604" v-if="scope.row.status < 3 " @click="mode(scope.row,'cancel')">取消</span>
            </div>
          </template>
        </el-table-column>
      </el-table>
