...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    let Satellite = inject("Satellite") as any;
    let Traffic = inject("Traffic") as any;
    ...
    let { centerPoint, selectedVeh, selectedAllVehs } = toRefs(props);
    ...
    nextTick(() => {
      ...
      watch(
        Satellite,
        (val) => {
          setSatellite(mapType.value, val as any);
        },
        {
          immediate: true,
        }
      ...
      watch(
        Traffic,
        (val) => {
          SetTraffic(mapType.value, val as any);
        },
        {
          immediate: true,
        }
      ...
      watch(
        ...
        (val: any) => {
          if (!val || val.length == 0) {
            dwawAssociateMarker([], InfoWindowRef as Ref<HTMLDivElement>);
          } else {
            let arr = [] as any;
            ...
            dwawAssociateMarker(arr, InfoWindowRef as Ref<HTMLDivElement>);
          }
        },
        {
          immediate: true,
