  ...
  setup(props, { emit }) {
    ...
    watch(
      ...
      (val) => {
        nextTick(async () => {
          if (val) {
            const dom = document.querySelector(".close") as any;
            if (videoFlag.value) {
              panoramaRef.value.style.height = `calc(100%)`;
              panoramaRef.value.style.right = 0;
              dom.style.top = `16px`;
            } else {
              panoramaRef.value.style.height = `calc(100% - 56px)`;
              panoramaRef.value.style.right = `10px`;
              dom.style.top = `70px`;
            }
