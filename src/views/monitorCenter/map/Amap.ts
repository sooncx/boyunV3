...
export function useAmap(mapRef: Ref<HTMLDivElement>, centerPoint = [114.410658, 33.113539], zoom = 10) {
    ...
    let TracePathpolyline = null as any; // 行程轨迹实例
            ...
            for (let i = 0; i < vehs.length; i++) {
                ...
                if (markerMap.has(nowVehInfo.V)) {
                    ...
                    if (
                        ...
                        nowVehInfo.iconType != oldVehInfo.iconType
                    ) {
    ...
    function zoomMarker(veh: any, InfoWindowRef: any, track = true) {
        nextTick(async () => {
            if (!veh || veh.Y == undefined || veh.X == undefined) return;
    ...
    function ClusterOpen(selectedAllVehs: any) {
        ...
        const renderMarker = function (context: any) {
            ...
            let item = data[0];
            const { lon, lat } = GPS.gcj_encrypt(parseFloat(item.Y), parseFloat(item.X));
            let content = `<img style="width: 32px;height:35px;transform: rotate(${item.F || 0
                }deg);" src="${getCarIcon(item)}" />`;
    ...
    // 行程轨迹
    function DrawTracePath(path: any[]) {
        map && map.clearMap && map.clearMap();
        TracePathpolyline && map.remove(TracePathpolyline);
        TracePathpolyline = null;
        if (path.length == 0) return;
        function newIcon(src: any) {
        ...
        //轨迹样式
        TracePathpolyline = new AMap.Polyline({
            strokeColor: "#1169f9", // 线颜色
        ...
        //开始图标
        const startMarker = newIcon(require("../../../assets/img/playback/track_start.png"));
        startMarker.setPosition(pathArr[0]);
        // 结束图标
        const endMarker = newIcon(require("../../../assets/img/playback/track_end.png"));
        endMarker.setPosition(pathArr[pathArr.length - 1]);
        map.add(startMarker);
        map.add(endMarker);
        // 创建折线实例
        TracePathpolyline.setPath(pathArr);
        // 将折线添加至地图实例
        map.add(TracePathpolyline);
        map.setFitView(TracePathpolyline, true);
    };
    // 追踪
