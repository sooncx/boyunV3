  ...
  >
    ...
    >
      ...
      <VideoControl
        v-bind="$attrs"
        :vehInfo="vehInfo"
        v-if="['直播控制'].includes(DialogType)"
      />
      <VideoLibrary
        v-bind="$attrs"
        :vehInfo="vehInfo"
        v-if="['录像库'].includes(DialogType)"
      />
      <SearchList
        v-bind="$attrs"
        :vehInfo="vehInfo"
...
export default defineComponent({
  components: {
    ...
    VideoLibrary: defineAsyncComponent(
      () => import("./contents/VideoLibrary/index.vue")
    ),
    TravelPath: defineAsyncComponent(
      () => import("./contents/TravelPath/index.vue")
    ),
    VideoControl: defineAsyncComponent(
      () => import("./contents/VideoControl/index.vue")
    ),
    SearchList: defineAsyncComponent(
      () => import("./contents/VideoPlayback/SerachList.vue")
    ),
  },
  props: {
    vehInfo: {
  ...
  setup(props, { emit }) {
    ...
    const width = computed(() => {
      if (["录像库", "行程轨迹"].includes(props.DialogType)) {
        return "1000px";
      } else if (["录像回放"].includes(props.DialogType)) {
        return "880px";
      } else if (["OBD数据"].includes(props.DialogType)) {
        return "680px";
      } else if (["无线回传"].includes(props.DialogType)) {
        return "640px";
      } else if (["工作模式", "直播控制"].includes(props.DialogType)) {
        return "600px";
      } else if (
        [
          "清除紧急报警",
