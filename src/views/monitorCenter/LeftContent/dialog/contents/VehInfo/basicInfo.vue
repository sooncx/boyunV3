  ...
  <div class="basicInfoRef">
    ...
    >
      ...
      <el-form-item label="图标">
        <div class="iconBar">
          <i
            v-for="(item, index) in getCarIcon({}, 'all')"
            :key="index"
            class="iconfont"
            :class="item.value"
            :title="item.label"
            :style="{color: formData.iconType == item.key?'blue':''}"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const data = reactive({
      ...
      selectIcon(val: any) {
        data.formData.iconType = val.key;
      },
      // 获取关联数据
      ...
      watch(
        ...
        (val) => {
          if (!val) {
            ...
            // 添加车辆时图标类型默认为后台返回列表第一个
            if (props.type == "add") {
              data.formData.iconType = getCarIcon({}, "all")[0].key;
            }
          }
        },
        {
