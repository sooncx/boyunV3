    ...
    <div class="VehItemBox">
      ...
      >
        ...
        >
          ...
          >
            <i
              class="iconfont carIcon"
              :class="getCarIcon(item, 'list')"
            ></i>
          ...
          <div
            class="vedio"
            v-if="item && item.T == 'GH08' && [1,2].includes(item.Z)"
          >
            <i
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const data = reactive({
      ...
      // 判断是否全选车辆列表所有可选车辆
      isChoseAllVeh() {
        const arr = data.tableData.filter((item: any) => {
          return data.selectVehId.has(item.V);
        });
        if (data.usingVehsCounts == arr.length && data.selectVehId.size != 0) {
          return true;
        }
        return false;
      },
      // 获取车辆图标
      getCarsIcon(carType: any) {
        switch (carType) {
          case 0:
            return "icon-car1";
            break;
    ...
    async function loadVehicles(lodaingFlag = true) {
      ...
      try {
        ...
        if (flag == 1) {
          ...
          data.vehlistMap = new Map(); // 当前车辆列表map集合
        ...
        setTimeout(() => {
          ...
          if (props.selectedVeh) {
            ...
            // copyVeh可能为underfined，例如当前选中车辆为在线车辆，此时切换到离线车辆列表，则在列表中找不到当前选中车辆
            // 所以copyVeh存在时才更新选中车辆的值
            copyVeh && emit("update:selectedVeh", copyVeh);
          }
          if (props.selectedAllVehs.size > 0) {
            let arrMap = new Map();
            [...props.selectedAllVehs.keys()].map((key: any) => {
              // data.vehlistMap.get(key) 可能为underfined，理由同上
    ...
    function handleOnVehItemClick(veh: any, flag: boolean) {
      ...
      if (flag) {
        ...
        const i = arr.findIndex((item) => item && item.V == veh.V);
        // 若取消车辆是当前选中车辆，则选中上次选中车辆
        if (veh.V && arr[0] && veh.V == arr[0].V) {
