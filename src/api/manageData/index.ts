...
export default {
   ...
   // 车辆续费导出
   exportVehicleExpire: (params: any): Promise<any> => GET(url.exportVehicleExpire, params), 
   // 续费记录导出
   vehicleFreeLogExport: (params: any): void => exportFile(url.vehicleFreeLogExport, params), 
   // 车币划拨记录导出
   userRechargeLogExport: (params: any): void => exportFile(url.userRechargeLogExport, params), 
   // 续费接口
   ...
   // 结清总数
   getSettleTotal: (): Promise<any> => GET(url.getSettleTotal),
   // 没有错误数数把数据全部插入表格中
   ...
   // 导出车组下所有车辆
   exportVehicleByGroupRds: (params: any): Promise<any> => GET(url.exportVehicleByGroupRds, params), 
   // 用户数据导出
