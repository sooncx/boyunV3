    ...
    function getList() {
      ...
      if (store.state.terminalResource && store.state.terminalList) {
        ...
        if ([...listData.value].length == 0) {
          ...
          // data.optionList.push({ name: "录像库" }); // 下个版本再开放
          data.optionList.push({ name: "行程轨迹" });
          // data.optionList.push({ name: "直播控制" });
          if (
      ...
      (val) => {
        nextTick(() => {
          ...
          if (val) {
            ...
            H = screenH - clientY;
            let buttomH = 0;
            if (data.optionList.length >= 9) {
              buttomH = 9 * 33 + 35;
            } else {
              buttomH = data.optionList.length * 33 + 35;
            }
            if (props.popType == "区域") {
              buttomH = 5 * 33 + 35;
            }
            if (popoverVisible.value) {
              ...
              if (H - buttomH <= 20) {
                ...
                dom.style.bottom = H - buttomH + 50 + "px";
              } else {
                dom.style.bottom = H - buttomH + "px";
              }
            }
          }
        });
