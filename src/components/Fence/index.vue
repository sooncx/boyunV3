...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const data = reactive({
      tipShow: false, // 提示框visible
      satelliteBtn: false, // 卫星图层开关标识
      searchValue: "" as any, // 绑车时输入框值
      choseData: new Map(), // 绑车时绑定栏已选车组车辆
      choseItems: new Set(), // 绑车时绑定栏当前选中车组车辆
      homeAddress: "" as any, // 停留点(家)地址
      companyAddress: "" as any, // 停留点(公司)地址
      something: "", // 地图模糊搜索框值
      areaName: "", // 区域名称
      stayTime: "0", // 停留时间
      areaType: 1, // 区域类型
      pointName: "", // 位置点名称
      pointType: "1", // 位置点类型
      TwoChargeRadius: 50 as any, // 二押点半径
      TwoChargeName: "", // 二押点名称
      changeFenceData: null as any, // 准备修改的围栏数据
      newPathId: null, // 新加围栏成功后该围栏的id
      bindFenceData: null as any, // 准备绑定的围栏数据
      stayPointsData: {
        company: { lng: null, lat: null, address: "" } as any,
        home: { lng: null, lat: null, address: "" } as any,
      }, // 停留点数据
      useVehInfo: true, // 是否传入设备信息标志
      clear() {
        data.tipShow = false;
    ...
    function toolFn(type: String) {
      switch (type) {
        ...
        case "卫星图":
          data.satelliteBtn
            ? AmapObj.setSatellite("Amap", false)
            : AmapObj.setSatellite("Amap", true);
